...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	//apologies in advance for this code - i am typing it in notepad lmao
	hdmi_ctrl->palette[color/2]
	if(color % 2 == 0) //even case
	{
		hdmi_ctrl->palette[color/2] &= 0b00000000000000000001111111111110; //clear it all out except for the bits that may be used
		hdmi_ctrl->palette[color/2] |= red<<9; //insert red, right shift it 9 times to bring to correct idx
		hdmi_ctrl->palette[color/2] |= blue<5; //insert blue, right shift it 9 times to bring to correct idx
		hdmi_ctrl->palette[color/2] |= green<<1; //insert green, right shift it 9 times to bring to correct idx

	}
	else
	{
		hdmi_ctrl->palette[color/2] &= 0b00000001111111111110000000000000; //clear it all out except for the bits that may be used
		hdmi_ctrl->palette[color/2] |= red<<21; //insert red, right shift it 9 times to bring to correct idx
		hdmi_ctrl->palette[color/2] |= blue<17; //insert blue, right shift it 9 times to bring to correct idx
		hdmi_ctrl->palette[color/2] |= green<<13; //insert green, right shift it 9 times to bring to correct idx
